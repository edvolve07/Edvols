...
import { ArrowRight, BarChart3, BookOpenCheck, Mic2 } from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  return (
    <section className="page-stack">
      ...
      <div className={`grid gap-4 md:grid-cols-${hasAptitude && hasInterview ? 4 : hasAptitude || hasInterview ? 2 : 1}`}>
        {hasAptitude ? (
          <>
        ...
        {hasInterview ? (
          <>
            ...
            <StatCard label="Total Reports" value={totalReports} tone="coral" />
          </>
        ) : null}
      </div>
        ...
        <section className="surface p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-black text-ink">Interview Analytics</h3>
            </div>
            <Link
              to="/reports"
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-md border border-slate-200 bg-slate-50/60 p-4">
              ...
              <p className="mt-2 text-2xl font-black text-ink">{interviewAnalytics.reports || 0}</p>
            </div>
            <div className="rounded-md border border-slate-200 bg-slate-50/60 p-4">
              <p className="text-xs font-bold uppercase text-slate-500">Average Interview Score</p>
              <p className="mt-2 text-2xl font-black text-ink">{interviewAnalytics.average_percentage || 0}%</p>
            </div>
            <div className="rounded-md border border-slate-200 bg-slate-50/60 p-4">
              <p className="text-xs font-bold uppercase text-slate-500">Latest ATS Score</p>
              <p className="mt-2 text-2xl font-black text-ink">{interviewAnalytics.latest_ats_score || 0}%</p>
            </div>
          </div>
          <div className="mt-4 divide-y divide-slate-100 overflow-hidden rounded-md border border-slate-200">
            {interviewAnalytics.recent_reports?.length ? (
              interviewAnalytics.recent_reports.slice(0, 5).map((report) => (
                ...
                >
                  <div>
                    <p className="font-bold text-ink">{report.role || 'Interview Report'}</p>
                    <p className="text-xs text-slate-500">
                      {report.domain || 'General'} · {report.generated_date || formatDateTime(report.created_at)}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-ink">{report.percentage || 0}%</p>
                    <p className="text-xs text-slate-500">
                ...
                {stats.recent_submissions?.length ? (
                  stats.recent_submissions.map((submission) => (
                    <tr key={submission.id} className="hover:bg-slate-50/70">
                      <td className="font-semibold text-ink">{submission.assessment_title}</td>
                      ...
                      <td className="font-bold text-ink">{submission.percentage}%</td>
                      <td>
