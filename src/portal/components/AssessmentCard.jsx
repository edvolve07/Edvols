...
export default function AssessmentCard({ assessment, action }) {
  return (
    <article className="group rounded-md border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-card-hover">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-black text-ink">{assessment.title}</h3>
          <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold">
            <span className="badge bg-indigo-50 text-indigo-700">{assessment.concept}</span>
