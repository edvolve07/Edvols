...
function MetricBar({ label, value }) {
  ...
  const color = value >= 7 ? "bg-accent-500" : value >= 5 ? "bg-amber-500" : "bg-red-500";
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6">
        ...
        <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">Start a live interview</h1>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {INTERVIEW_DOMAINS.map((item) => (
                <button
                  ...
                  className={clsx(
                    "rounded-lg border px-4 py-3 text-left text-sm font-medium transition-all duration-150",
                    domain === item
          ...
          <section className="card p-5">
            ...
            <div className="mt-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-400">or type a custom role</span>
                <div className="h-px flex-1 bg-slate-200" />
              </div>
              <input
                type="text"
                ...
                className={clsx(
                  "field mt-3",
                  isCustomRole && "border-brand-300 bg-brand-50 text-brand-700",
                )}
              />
            </div>
          </section>
        ...
        <aside className="card p-5">
          ...
          <div
            ...
            className={clsx(
              "flex min-h-36 flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed p-5 text-center transition",
          ...
          >
            {file ? (
              <>
                ...
                <button onClick={() => setFile(null)} className="text-xs font-semibold text-brand-700 hover:text-brand-800">
                  Replace file
                </button>
              </>
            ) : (
              <>
                ...
                <label className="cursor-pointer text-sm font-semibold text-brand-700 hover:text-brand-800">
                  Browse files
                  <input
                    type="file"
          ...
          {error && (
            <div className="mt-4 flex gap-2 rounded-lg border border-red-100 bg-red-50 p-3 text-sm text-red-600">
        ...
        <section className="card overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
            ...
            <div className="flex items-center gap-3">
              {tabWarnings > 0 && (
                <span className="rounded-lg bg-amber-50 px-3 py-2 font-mono text-xs font-semibold text-amber-700">
                  {tabWarnings}/3 tab switches
                </span>
              )}
              <span className="rounded-lg bg-slate-100 px-3 py-2 font-mono text-sm font-semibold text-slate-700">
                {sessionTime}
              </span>
              <button
                onClick={onEnd}
                className="rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-100 active:scale-95"
              >
                End
              </button>
            </div>
          ...
          <div className="grid gap-px bg-slate-100 lg:grid-cols-2">
            <div className="relative min-h-[340px] bg-slate-950">
              ...
              <div className="absolute left-4 top-4 flex items-center gap-2 rounded-lg bg-black/55 px-3 py-2 text-xs font-semibold text-white backdrop-blur">
                {recorder.hasCamera ? <Video size={14} /> : <VideoOff size={14} />}
                {isRecording ? "Recording" : "Live"}
              </div>
              <div className="absolute bottom-4 left-4 rounded-lg bg-black/55 px-3 py-2 text-xs font-semibold text-accent-200 backdrop-blur">
              ...
              {session.skills_found.length > 0 && (
                <div className="flex flex-wrap gap-2 border-t border-slate-100 p-5">
                  {session.skills_found.map((skill) => (
                    <span key={skill} className="rounded-lg bg-accent-50 px-2 py-1 text-xs font-medium text-accent-700">
                      {skill}
                    </span>
                  ))}
                </div>
          ...
          <div className="grid gap-px bg-slate-100 lg:grid-cols-2">
            <div className="bg-slate-950 p-5">
              <div className="relative mb-5 h-36 overflow-hidden rounded-xl bg-slate-900">
                <canvas
                  ref={waveRef}
                  width={700}
                  height={200}
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                {loading ? (
                  <span className="inline-flex items-center gap-2 text-sm text-slate-400">
                    <Loader2 size={16} className="animate-spin text-accent-400" /> Evaluating answer
                  </span>
                ) : isReviewing ? (
                  <>
                    <button
                      onClick={recorder.reRecord}
                      className="inline-flex items-center gap-2 rounded-lg bg-slate-800 px-4 py-3 text-sm font-semibold text-slate-200 transition hover:bg-slate-700 active:scale-95"
                    >
                      <RotateCcw size={16} /> Re-record
                    </button>
                    <button
                      onClick={recorder.submitAnswer}
                      className="inline-flex items-center gap-2 rounded-lg bg-accent-500 px-4 py-3 text-sm font-semibold text-white shadow-card transition hover:bg-accent-600 active:scale-95"
                    >
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-red-500 text-white shadow-lg transition hover:bg-red-600 active:scale-95"
                    >
                  ...
                  <button
                    ...
                    className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-accent-500 text-white shadow-card transition hover:bg-accent-600 active:scale-95"
                  >
                    <Mic size={22} />
                  </button>
                )}
              ...
              {recorder.errMsg && (
                <p className="mt-4 rounded-lg border border-red-500/20 bg-red-500/10 p-3 text-center text-sm text-red-200">
                  {recorder.errMsg}
                </p>
              )}
            </div>
            ...
            <div className="bg-white p-5">
              ...
              {error && (
                <div className="mb-4 flex gap-2 rounded-lg border border-red-100 bg-red-50 p-3 text-sm text-red-600">
                  <AlertCircle size={16} className="mt-0.5 shrink-0" /> {error}
                </div>
              )}
              {!loading && !transcript && !feedback && (
                <div className="flex min-h-36 flex-col items-center justify-center rounded-xl bg-slate-50 text-center text-sm text-slate-400">
                  <Mic2 size={26} className="mb-2" />
                  Submit an answer to see transcript, feedback, and metrics.
                </div>
              )}
              {transcript && (
                <div className="mb-4 rounded-lg bg-accent-50 p-4 text-sm leading-6 text-accent-900">
                  <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-accent-500">Transcript</p>
                  {transcript}
                </div>
              )}
              {feedback && (
                <div className="mb-4 rounded-lg bg-slate-50 p-4 text-sm leading-6 text-slate-700">
          ...
          <div className="card p-5">
            ...
            <div className="mt-4 grid grid-cols-10 gap-1">
              {Array.from({ length: 10 }).map((_, index) => (
                <span
                  key={index}
                  className={clsx("h-2 rounded-full", index < questionNumber ? "bg-accent-500" : "bg-slate-200")}
                />
              ))}
            </div>
  ...
  if (phase === "complete" && session) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-xl flex-col items-center justify-center px-4 py-10 text-center">
        <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-accent-50 text-accent-500">
          <CheckCircle2 size={42} />
        </div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">Interview complete</h1>
        <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
          Your answers have been recorded. Generate the final report to review scores and improvement areas.
        </p>
        {error && (
          <div className="mt-5 flex gap-2 rounded-lg border border-red-100 bg-red-50 p-3 text-sm text-red-600">
            <AlertCircle size={16} className="mt-0.5 shrink-0" /> {error}
          </div>
        )}
        <button
