...
export async function downloadResumeLatex(versionId, version = "") {
  const res = await fetch(`${BASE_URL}/api/student/resume-builder/versions/${versionId}/pdf?format=latex`, { headers: authHeaders() });
  if (!res.ok) throw new Error(await res.text());
  const blob = await res.blob();
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `resume${version ? `_v${version}` : ""}.tex`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
